...
async def run_intake(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if not content:
        return {**state, "errors": state.get("errors", []) + ["No images or description provided"]}
    ...
    try:
        ...
        log.error("intake.json_parse_error", raw=raw[:200])
        return {**state, "errors": state.get("errors", []) + [f"Failed to parse AI response: {raw[:100]}"]}
